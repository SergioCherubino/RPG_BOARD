...
function getTableCellsRect(startX, startY, horizontal) {
  const cells = [];
...
function placeTableObject() {
  ...
  while (!placed) {
    ...
    for (const c of cells) {
      grid[c.y][c.x] = { type: "object", solid: true };
    }

    // renderiza UMA imagem sobrepondo os tiles
    const cell = getCell(start.x, start.y);
